...
\begin{enumerate}
  \item Assemble and normalize a working collection of Sorani news text from an existing, openly published dataset.
...
The primary dataset is the Kurdish News Dataset Headlines (KNDH), an openly published multiclass collection of Sorani news headlines distributed through \emph{Data in Brief}~\cite{badawiKurdishNewsDataset2023}. Where additional running text is useful for evaluation, the project may draw on the publicly documented AsoSoft text corpus~\cite{veisiKurdishLanguageProcessing2019}. Both sources are used as-is, with provenance preserved.

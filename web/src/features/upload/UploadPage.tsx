  ...
  return <div className="h-full overflow-y-auto bg-canvas">
    <div className="mx-auto max-w-4xl p-4 md:p-6">
      ...
      <div className="mt-5 grid gap-5">
        ...
        {selected ? <form className="overflow-hidden rounded-xl bg-surface shadow-sm" onSubmit={(event) => {event.preventDefault(); start();}}>
          <section>
            <div className="flex items-center gap-3 px-4 pt-4"><span className="grid size-7 place-items-center rounded-md bg-surface-raised text-caption font-semibold">02</span><SlidersHorizontal className="size-4" /><Typography variant="label">Document rules</Typography></div>
            <div className="grid gap-4 p-4 sm:grid-cols-2">
              ...
              {selected.kind === "text" ? <FormField label="Split documents by"><SegmentedControl className="inline-flex w-auto self-start" aria-label="Split documents by" value={split} options={[{value: "Line", label: "Line"}, {value: "Paragraph", label: "Paragraph"}]} onValueChange={setSplit} /></FormField> : <><FormField label="Text column" required><Select aria-label="Text column" value={textCol} options={selected.columns.map((value) => ({value, label: value}))} onValueChange={setTextCol} /></FormField><FormField label="Label column"><Select aria-label="Label column" value={labelCol} options={[{value: "(none)", label: "No label"}, ...selected.columns.map((value) => ({value, label: value}))]} onValueChange={setLabelCol} /></FormField></>}
            </div>
          </section>

          <section>
            <div className="flex items-center gap-3 px-4 pt-3"><span className="grid size-7 place-items-center rounded-md bg-surface-raised text-caption font-semibold">03</span><Settings2 className="size-4" /><Typography variant="label">Model and fit</Typography></div>
            <div className="p-4">
              ...
              <div className="mt-2 grid gap-x-6 gap-y-1 md:grid-cols-3">
                <div className="py-2"><Switch checked={normalize} onCheckedChange={setNormalize} label="KLPT normalization" description="Normalize raw Sorani" /></div>
                <div className="py-2"><Switch checked={autoMcs} onCheckedChange={setAutoMcs} label="Auto granularity" description="Scale clusters to corpus" /></div>
                <div className="py-2"><Switch checked={baselines} onCheckedChange={setBaselines} label="LDA/NMF baselines" description="Add comparisons" /></div>
              </div>
          ...
          <footer className="mt-1 bg-surface-raised px-4 py-4">
            {!jobId && (
